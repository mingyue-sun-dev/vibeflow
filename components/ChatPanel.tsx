      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full gap-3 text-center pb-8">
            ...
            <div className="space-y-1">
              <p className="text-zinc-400 text-sm font-medium">Shape your playlist</p>
              <p className="text-zinc-600 text-xs">
                Try &ldquo;make it more calm&rdquo;, &ldquo;only upbeat tracks&rdquo;,
              </p>
              <p className="text-zinc-600 text-xs">or use the quick controls →</p>
            </div>
          </div>
        )}
        ...
        {isTransforming && (
          <div className="flex flex-col gap-1.5 items-start animate-fade-in">
            <span className="text-xs text-zinc-600 pl-1">Refining your playlist…</span>
      ...
      <div className="px-4 py-4 border-t border-zinc-800 shrink-0">
        <div className="flex gap-2">
          ...
          {isTransforming ? (
            <button
              onClick={onCancel}
              className="bg-zinc-700 text-zinc-300 rounded-lg px-4 py-2.5 text-sm font-medium hover:bg-zinc-600 hover:text-white active:scale-95 transition-all shrink-0"
            >
              Cancel
            </button>
          ) : (
